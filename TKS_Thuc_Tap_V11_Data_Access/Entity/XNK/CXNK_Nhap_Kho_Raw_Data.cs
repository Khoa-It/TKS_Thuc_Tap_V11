...
namespace TKS_Thuc_Tap_V11_Data_Access.Entity.XNK
{
    public class CXNK_Nhap_Kho_Raw_Data
    {
        ...
        private DateTime? m_dtmNgay_Nhap;
        ...
        public DateTime? Ngay_Nhap
        {
            get
            {
                return m_dtmNgay_Nhap;
            }
            set
            {
                m_dtmNgay_Nhap = value;
            }
        }

        public string Ngay_Nhap_String
        {
            get
            {
                return m_dtmNgay_Nhap.HasValue ? m_dtmNgay_Nhap.Value.ToString("dd/MM/yyyy") : string.Empty;
            }
        }
        ...
        public double Tri_Gia
        {
            get
            {
                return m_dblSL_Nhap * m_dblDon_Gia_Nhap;
            }
            set
            {
                m_dblTri_Gia = value;
        ...
        public bool IsValid(out string p_strErrorMessage)
        {
            ...
            else if (m_dblSL_Nhap <= 0)
            {
                p_strErrorMessage = "Trường số lượng nhập không được nhỏ hơn hoặc bằng 0";
                return false;
            }
            else if (m_dblDon_Gia_Nhap < 0)
            {
                p_strErrorMessage = "Trường đơn giá nhập không được nhỏ hơn 0";
                return false;
            }
            p_strErrorMessage = "";
            return true;

...
namespace TKS_Thuc_Tap_V11_Data_Access.Controller.Cache
{
    public class CCache_San_Pham
    {
        ...
        public static CDM_San_Pham Get_Data_By_Ma_SP(string p_strCode)
        {
            if (string.IsNullOrWhiteSpace(p_strCode))
                return null;

            p_strCode = p_strCode.Trim();

            foreach (var item in Dic_Data_Code.Values)
            {
                if (item.Ma_San_Pham?.Trim() == p_strCode)
                    return item;
            }
